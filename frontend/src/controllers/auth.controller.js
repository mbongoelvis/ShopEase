...
// GET /employees — list all employees for the current owner's store
export async function listEmployees(req, res) {
  const storeId = req.user.storeId;
  
  try {
    const result = await pool.query(
      "SELECT user_id, user_name, email, role, created_at FROM user_account WHERE store_id = $1 AND role <> 'OWNER' ORDER BY created_at DESC",
      [storeId]
    );
